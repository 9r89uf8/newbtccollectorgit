...
export async function getDashboardData() {
  ...
  try {
    const [
      ...
      query(`
        select
          ...
          coalesce(sum(book_ticker_update_count), 0)::int as book_ticker_updates,
          coalesce(sum(liquidation_count), 0)::int as liquidation_count,
          avg(spread_bps_avg) as avg_spread_bps
    ...
    return {
      ...
      websocketStats: websocketStats.rows[0] || null,
      recentWebSocketSummaries: recentWebSocketSummaries.rows,
