    ...
    const [
      ...
      query(
        `
          ...
          join markets probability_market on probability_market.id = p.market_id
          where p.market_id = any($1::text[])
            and p.scheduled_at >= $2
            and p.scheduled_at < $3
            and p.scheduled_at >= probability_market.start_time
            and p.scheduled_at < probability_market.end_time
            and p.sample_type <> 'preopen'
          order by p.scheduled_at asc, p.market_id asc
        `,
        [marketIds, windowStart, windowEnd]
      ),
    ]);
