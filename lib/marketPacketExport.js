...
const SCHEMA_VERSION = "btc_5m_market_packet_v2";
...
function compactFeatureBucket(bucket, startMs) {
  return {
    ...
    cvd_5m_quote: toNumber(bucket.cvd_market_quote),
...
function buildRows1s(data, startMs, seconds, openInterestBase) {
  ...
  for (let second = 0; second < seconds; second += 1) {
    ...
    rows.push({
      ...
      cvd_5m_quote: toNumber(flow?.cvd_market_quote),
      cvd_30s_quote: toNumber(flow?.rolling_net_30s),
  ...
  return {
    ...
    packet: {
      ...
      notes: {
        ...
        cvd_5m_quote: "Market-local cumulative volume delta: running sum of aggressive buy quote minus aggressive sell quote over this 5-minute market.",
        cvd_30s_quote: "Rolling CVD over the available trailing 30 seconds. It starts immediately with a partial window and becomes a full 30-second window at s=30.",
      },
      market: {
        id: data.market.id,
